...
import psutil
from userbot import ALIVE_LOGO, ALIVE_NAME, BOT_VER, CMD_HELP, GEEZ_TEKS_KUSTOM, StartTime, bot
...
@register(outgoing=True, pattern=r"^\.(?:peyalive)\s?(.)?")
...
@register(outgoing=True, pattern=r"^\.(?:apeyy)\s?(.)?")
async def amireallyalive(alive):
    await bot.get_me()
    await get_readable_time((time.time() - StartTime))
    output = (
        f"**ㅤㅤㅤㅤ ✯卩乇ㄚㄚ - 𝑃 𝑅 𝑂 𝐽 𝐸 𝐶 𝑇 ✯**\n"
        f"╔══════════╬════╬══════════╗ \n"
